...
function OrederExecute(props) {
  ...
  const clearAllOrder = async()=>{
    ...
    try {
      const result = await fetch(`${baseUrl}/api/exeOrders/clearOreders`, option);
  ...
  return (
    <OrederExecuteContext.Provider value={{exeOrders, setExeOrderCount,addExeOreder,clearAllOrder}}>
      {props.children}
